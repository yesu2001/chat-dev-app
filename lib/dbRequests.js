...
export const addNewMember = async (userData, groupId) => {
  try {
    ...
    // const {dataone,errorone} = await supabase.from("members").select().eq("user_id",userData?.id)
...
// Search for groups names
export const searchGroup = async (text) => {
  try {
    const { data, error } = await supabase
      .from("groups")
      .select()
      .ilike("name", `%${text}%`);
    if (error) {
      throw new Error("Error Findig group names to DB :-", error);
    }
